-- PetID — Ao confirmar resgate (leitura QR com ocorrência aberta),
-- abre conversa e envia mensagem automática do finder para o tutor
-- ver o sinalizador (badge) no ícone de chat.
...
begin
  ...
  -- Evita mensagem duplicada se o trigger disparar mais de uma vez
  select exists (
    select 1
    from public.mensagens m
    where m.conversa_id = v_conversa_id
      and m.autor = 'finder'
      and m.corpo like '%tag PetID%'
  ...
  if new.tipo_evento = 'qr_lido_com_localizacao' then
    if v_endereco is not null then
      v_corpo := format(
        'Confirmei o resgate de %s pela tag PetID. Local aproximado: %s',
        v_animal_nome,
        v_endereco
      );
    else
      v_corpo := format(
        'Confirmei o resgate de %s pela tag PetID. Compartilhei a localização aproximada.',
        v_animal_nome
      );
    end if;
  else
    v_corpo := format(
      'Confirmei o resgate de %s pela tag PetID.',
      v_animal_nome
    );
  end if;
...
drop trigger if exists trg_notificacao_qr_abre_chat on public.notificacoes;
create trigger trg_notificacao_qr_abre_chat
  after insert on public.notificacoes
  for each row
  execute function public.abrir_chat_aviso_leitura_qr();

comment on function public.abrir_chat_aviso_leitura_qr() is
  'Após notificação de leitura QR, cria conversa + mensagem automática para o badge do chat do tutor.';
